...
public class Heap {
    ...
    public void siftDown() {
        ...
        while (hasLeftChild(idx)) {
            ...
            int lc = items[lIdx];
            int rc = isMax ? rIdx < size ? items[rIdx] : Integer.MIN_VALUE
                    : rIdx < size ? items[rIdx] : Integer.MAX_VALUE;
        ...
        /*
         * int[] arr= Arrays.copyOf(items,size); if (HeapArrImpl.isMinHeap(arr, 0)) {
         * InputUtil.print("Min HeapArrImpl -\tYes", arr); } else if
         * (HeapArrImpl.isMaxHeap(arr, size)) {
